...
#module name
name="aproofbtc"
class aproofbtc(aproofbase):

    def __init__(self, name = "vproof", 
            ttype = "btc", 
            dtype = "btcxxx",       #metadata  type:v2lxxx/l2vxxx
        ...
        super().__init__(name, ttype, dtype, dbconf, fdbconf, rdbconf, nodes, "btc")
    ...
    def update_proof_info(self, tran_info):
        try:
            ...
            new_proof = new_proofstate in (self.proofstate.START, self.proofstate.PRESTART)
            ...
            if new_proof:
                ...
                if found: 
                    ...
                    txid    = db_tran_info.get("txid", "")
                    version = db_tran_info.get("version", -1)
                    new_proof = False
                    if txid != tran_info["txid"]:
                ...
                ret = self._dbclient.get_proof_by_hash(tran_id)
                if ret.state != error.SUCCEED:
                    return result(error.TRAN_INFO_INVALID, f"get_proof_by_hash({tran_id}) failed.")
