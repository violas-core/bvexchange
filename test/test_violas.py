...
def new_account_for_address(wclient):
    ...
    return ret.datas.auth_key_prefix.hex() + ret.datas.address.hex()
...
def _test_send_tran_exg(vclient, wclient, ttype, dtype, sender, receiver, token_id, module, toaddress, amount = 10000):
    ...
    if module is None:
        ret = vclient.send_platform_coin(account, receiver, amount, tran_data)
    else:
        ret = vclient.send_violas_coin(account, receiver, amount, token_id, module, tran_data)
...
def _get_platform_coin(client, receiver, amount = 1000000):
    ...
    ret = wclient.get_account(sender)
    assert ret.state == error.SUCCEED, f"get ({sender}'s account failed.{ret.message})"
...
def run():
    try:
        ...
        comm_funs.init_address_list(vclient, wclient, addresses_violas, vlibra_module, vlibra_id, module)
        addr_list.extend(addresses_violas)

        #v2l 
        logger.debug("\n\n****test send tran exg(violas)\n")
        _test_send_tran_exg(vclient, wclient, "violas", "v2l", addresses_violas[0], receivers_v2l[0], vlibra_id, module, addresses_libra[0])
        
        #test l2v
        logger.debug("\n\n****test send tran exg(libra)\n")
        _test_send_tran_exg(lclient, wclient, "libra", "l2v", addresses_libra[0], receivers_l2v[0], None, None, addresses_violas[0])
