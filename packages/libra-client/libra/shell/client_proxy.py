...
class ClientProxy:
    def __init__(self, client: Client, libra_args):
    ...
    def get_latest_account_state(self, address_or_refid):
        address = self.parse_address_or_refid(address_or_refid)
        return self.grpc_client.get_account_blob(address)
    ...
    def get_committed_txn_by_acc_seq(self, address_or_refid, seq, fetch_events):
        ...
        return self.grpc_client.get_account_transaction(address, seq, fetch_events)
    ...
    def violas_publish(self,sender_address_or_refid, is_blocking):
        sender_account = self.address_or_refid_to_account(sender_address_or_refid)
        self.grpc_client.violas_publish(sender_account, is_blocking)

    def violas_init(self,sender_address_or_refid, module_address_or_refid, is_blocking):
        sender_account = self.address_or_refid_to_account(sender_address_or_refid)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        self.grpc_client.violas_init(sender_account, module_address, is_blocking)

    def violas_mint_coin(self, receiver_address_or_refid, violas, module_address_or_refid, is_blocking):
        ...
        self.grpc_client.violas_mint_coin(receiver_address, micro_violas, module_account, is_blocking)

    def violas_transfer_coin(self, sender, recevier, coin, module_address_or_refid, max_gas, unit_price, is_blocking):
        sender = self.address_or_refid_to_account(sender)
        recevier = self.parse_address_or_refid(recevier)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        micro_libra = Uint64.int_safe(coin) * 1_000_000
        self.grpc_client.violas_transfer_coin(sender, bytes.fromhex(recevier), micro_libra, bytes.fromhex(module_address), max_gas, unit_price, is_blocking)
        return sender.sequence_number

    def violas_make_order(self, sender_address_or_refid, module_address_or_refid, amount, price, is_blocking):
        sender_account = self.address_or_refid_to_account(sender_address_or_refid)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        self.grpc_client.violas_make_order(sender_account, bytes.fromhex(module_address), Uint64.int_safe(amount) * 1_000_000, Uint64.int_safe(price) * 1_000_000, is_blocking=is_blocking)
        return sender_account.sequence_number

    def violas_pick_order(self, sender_address_or_refid, order_address_or_refid, module_address_or_refid, is_blocking):
        sender_account = self.address_or_refid_to_account(sender_address_or_refid)
        order_address = self.parse_address_or_refid(order_address_or_refid)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        self.grpc_client.violas_pick_order(sender_account, bytes.fromhex(module_address), bytes.fromhex(order_address), is_blocking=is_blocking)
        return sender_account.sequence_number

    def violas_withdraw_order(self, sender_address_or_refid, module_address_or_refid, is_blocking):
        sender_account = self.address_or_refid_to_account(sender_address_or_refid)
        module_address = self.parse_address_or_refid(module_address_or_refid)
        self.grpc_client.violas_pick_order(sender_account, bytes.fromhex(module_address), bytes.fromhex(module_address), is_blocking=is_blocking)
        return sender_account.sequence_number
    ...
    def get_violas_balance(self, account_address_or_refid,module_address_or_refid):
        ...
        module_address = self.parse_address_or_refid(module_address_or_refid)
        micro_libra = self.grpc_client.violas_get_balance(bytes.fromhex(account_address),bytes.fromhex(module_address))
        return micro_libra / 1_000_000
