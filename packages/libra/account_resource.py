...
class AccountState(Struct):
    ...
    def get_resource(self):
        resource = self.ordered_map.get(AccountConfig.account_resource_path())
        if resource:
    ...
    def to_violas_json_serializable(self, module_address):
        amap = super().to_json_serializable()
        ar = self.get_resource()
        info = self.get_violas_info(module_address.hex())
        resource = self.get_violas_resource(module_address.hex())
        ret = {}
        if ar:
            ret.update(ar.to_json_serializable())
        if info:
            ret.update(json.loads(info.to_json()))
        if resource:
            ret.update(json.loads(resource.to_json()))
        return ret
