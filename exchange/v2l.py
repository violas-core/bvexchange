...
class v2l(exbase):    
    ...
    def exec_exchange(self, data, from_sender, map_sender, combine_account, receiver, \
        ...
        #if found transaction in history.db, 
        ...
        if self.use_module(state, localdb.state.QBSUCCEED):
            if not detail.get("swap_version"):
                ...
                out_amount = self.amountswap(out_amount, \
                        self.amountswap.amounttype[self.map_chain.upper()]).violas_amount

                self._logger.debug(f"exec_exchange-0.result : can swap amount: = {out_amount_chian} gas = {gas}, want = {out_amount}{to_token_id}")
            ...
            if ret.state != error.SUCCEED:
                ...
                return result(error.FAILED, "get_swap_balance failed.")
            else:
                #diff balance
        ...
        if self.use_module(state, localdb.state.FILLSUCCEED) or \
            ...
            self._logger.debug(f"exec_exchange-4. start send {to_token_id} to {toaddress} amount = {send_amount}...")
        ...
        if self.use_module(state, localdb.state.VSUCCEED):
            ...
            ret = self.send_coin_for_update_state_to_end(from_sender, receiver, tran_id, from_token_id, 1, out_amount_real = self.amountswap(detail["diff_balance"]).microamount(self.map_chain))
            if ret.state != error.SUCCEED:
                return ret
