...
class v2b(vlbase):    
    ...
    def exec_exchange(self, data, from_sender, map_sender, combine_account, receiver, \
        ...
        #send libra token to payee address. P = payee
            markdata = self.btc_client.create_data_for_mark(self.libra_chain, self.dtype, \
                    tran_id, version)
        ...
        if self.use_module(state, localdb.state.VSUCCEED):
            self.send_coin_for_update_state_to_end(from_sender, receiver, tran_id, from_token_id, version = version)
